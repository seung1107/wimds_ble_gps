    ...
    *******************************************************************************/
    void UART_GPS_UartInit(const UART_GPS_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            UART_GPS_rxBufferSize  = (uint8) config->rxBufferSize;

            /* Set TX direction internal variables */
            UART_GPS_txBuffer      =         config->txBuffer;
            UART_GPS_txDataBits    = (uint8) config->dataBits;
            UART_GPS_txBufferSize  = (uint8) config->txBufferSize;
...
void UART_GPS_UartPostEnable(void)
{
    ...
    if (UART_GPS_CHECK_TX_SCL_MISO_PIN_USED)
    {
        /* Set SCB UART to drive the output pin */
        UART_GPS_SET_HSIOM_SEL(UART_GPS_TX_SCL_MISO_HSIOM_REG, UART_GPS_TX_SCL_MISO_HSIOM_MASK,
                                       UART_GPS_TX_SCL_MISO_HSIOM_POS, UART_GPS_HSIOM_UART_SEL);
    }
#endif /* (UART_GPS_TX_SCL_MISO_PIN_PIN) */
    ...
    if (UART_GPS_CHECK_RTS_SS0_PIN_USED)
    {
        ...
        UART_GPS_SET_HSIOM_SEL(UART_GPS_RTS_SS0_HSIOM_REG, UART_GPS_RTS_SS0_HSIOM_MASK,
                                       UART_GPS_RTS_SS0_HSIOM_POS, UART_GPS_HSIOM_UART_SEL);
    }
...
void UART_GPS_UartStop(void)
{
        ...
        if (UART_GPS_CHECK_TX_SCL_MISO_PIN_USED)
        {
            ...
            UART_GPS_SET_HSIOM_SEL(UART_GPS_TX_SCL_MISO_HSIOM_REG, UART_GPS_TX_SCL_MISO_HSIOM_MASK,
                                           UART_GPS_TX_SCL_MISO_HSIOM_POS, UART_GPS_HSIOM_GPIO_SEL);
        }
    #endif /* (UART_GPS_TX_SCL_MISO_PIN_PIN) */
        ...
        if (UART_GPS_CHECK_RTS_SS0_PIN_USED)
        {
            ...
            /* Set GPIO to drive output pin */
            UART_GPS_SET_HSIOM_SEL(UART_GPS_RTS_SS0_HSIOM_REG, UART_GPS_RTS_SS0_HSIOM_MASK,
                                           UART_GPS_RTS_SS0_HSIOM_POS, UART_GPS_HSIOM_GPIO_SEL);
        }
        ...
        /* Set GPIO to drive output pin */
        UART_GPS_SET_HSIOM_SEL(UART_GPS_TX_HSIOM_REG, UART_GPS_TX_HSIOM_MASK,
                                       UART_GPS_TX_HSIOM_POS, UART_GPS_HSIOM_GPIO_SEL);
    #endif /* (UART_GPS_UART_TX_PIN) */
        ...
        /* Set GPIO to drive output pin */
        UART_GPS_SET_HSIOM_SEL(UART_GPS_RTS_HSIOM_REG, UART_GPS_RTS_HSIOM_MASK,
                                       UART_GPS_RTS_HSIOM_POS, UART_GPS_HSIOM_GPIO_SEL);
    #endif /* (UART_GPS_UART_RTS_PIN) */
...
void UART_GPS_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *******************************************************************************/
    void UART_GPS_UartSaveConfig(void)
    {
        ...
        */
        ...
        UART_GPS_UART_RX_CTRL_REG |= UART_GPS_UART_RX_CTRL_SKIP_START;

    #if(UART_GPS_SCB_MODE_UNCONFIG_CONST_CFG)
        #if(UART_GPS_MOSI_SCL_RX_WAKE_PIN)
            (void) UART_GPS_uart_rx_wake_i2c_sda_spi_mosi_ClearInterrupt();
        #endif /* (UART_GPS_MOSI_SCL_RX_WAKE_PIN) */
    #else
    ...
    *******************************************************************************/
    CY_ISR(UART_GPS_UART_WAKEUP_ISR)
    {
        ...
        * only disabled by UART_GPS_UartRestoreConfig() call.
        */
    #if(UART_GPS_SCB_MODE_UNCONFIG_CONST_CFG)
        #if(UART_GPS_MOSI_SCL_RX_WAKE_PIN)
            (void) UART_GPS_uart_rx_wake_i2c_sda_spi_mosi_ClearInterrupt();
